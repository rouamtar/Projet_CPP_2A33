...
QSqlQueryModel* Audience ::rechercher_salle(int salle){
           QSqlQuery query;
           query.prepare(QString("SELECT * FROM AUDIENCES WHERE salle=:salle"));
           query.bindValue(":salle",salle);
           query.exec();
             QSqlQueryModel* model=new QSqlQueryModel();
...
bool Audience::valider(QString id)
{


 QSqlQuery query1;
 int n=0;
 query1.prepare("select count(ID_CARTE) from CARTE where ID_CARTE like '"+id+"%' ");
 query1.exec();
 while(query1.next())
     n++;
if(n==0)
{
query1.prepare("insert into carte values :id");
